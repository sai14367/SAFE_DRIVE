...
CONFIG = {
    ...
    'EMAIL_RECEIVER': "mvskgamer67@gmail.com",
...
# --- CORRECTED send_email FUNCTION ---
def send_email(subject, body, frame=None):
    """Sends an email alert with optional image attachment."""
    if not CONFIG['EMAIL_PASSWORD']:
        logging.warning("Email password not configured, skipping email alert")
        return
    ...
    if frame is not None:
        try:
            ...
            if ret:
                ...
                image = MIMEImage(image_data, name=f"{subject.replace(' ', '_')}.jpg")
                msg.attach(image)
...
# Global variables for the safety system
class SafetySystem:
    def __init__(self):
        self.is_running = False
        self.cap = None
        self.buzzer_process = None
...
def start_buzzer():
    if safety_system.buzzer_process is None or not safety_system.buzzer_process.is_alive():
        if os.path.exists(CONFIG['BUZZER_SOUND_FILE']):
            safety_system.buzzer_process = multiprocessing.Process(target=play_buzzer_process, args=(CONFIG['BUZZER_SOUND_FILE'],))
            safety_system.buzzer_process.daemon = True
            safety_system.buzzer_process.start()
        else:
            logging.error(f"Buzzer sound file not found: {CONFIG['BUZZER_SOUND_FILE']}")

def stop_buzzer():
    if safety_system.buzzer_process is not None and safety_system.buzzer_process.is_alive():
        try:
            safety_system.buzzer_process.terminate()
            safety_system.buzzer_process.join(timeout=1)
        except Exception as e:
            logging.error(f"Error terminating buzzer process: {e}")
        safety_system.buzzer_process = None

def trigger_alarm(text):
    """Prints and speaks an alert."""
    logging.warning(f"!!! ALERT: {text} !!!")
    safety_system.system_stats['last_alert'] = text
    safety_system.system_stats['alerts_sent'] += 1
    try:
        engine = pyttsx3.init()
        engine.say(text)
        engine.runAndWait()
...
def detection_loop():
    ...
    try:
        with mp_face_mesh.FaceMesh(
            ...
            while safety_system.is_running:
                ret, frame = safety_system.cap.read()
                if not ret:
                    break
                ...
                if not math_problem_state['active'] and not move_aside_state['given']:
                    if is_drowsy:
                        ...
                        if current_time - last_alert_time['drowsiness_voice'] > CONFIG['VOICE_ALERT_COOLDOWN_SECONDS']:
                            ...
                            # CORRECTED CALL
                            threading.Thread(target=send_email, args=("Sleeping Alert!", "Driver has been detected sleeping.", safety_system.current_frame)).start()
                ...
                if move_aside_state['given'] and not math_problem_state['active']:
                    ...
                    if current_time - move_aside_state['last_prompt_time'] > CONFIG['MOVE_ASIDE_DELAY']:
                        if move_aside_state['count'] < CONFIG['MOVE_ASIDE_REPETITIONS']:
                            ...
                            # CORRECTED CALL
                            threading.Thread(target=send_email, args=("Safety Instruction: Pull Over!", "Driver has been instructed to pull over to solve a problem.", safety_system.current_frame)).start()
                ...
                # Yawn, Phone, and Face Missing Alerts
                if not math_problem_state['active'] and not move_aside_state['given'] and is_yawning:
                    ...
                    if current_time - last_alert_time['yawn_voice'] > CONFIG['VOICE_ALERT_COOLDOWN_SECONDS']:
                        ...
                        # CORRECTED CALL
                        threading.Thread(target=send_email, args=("Yawn Alert", "Driver has been detected yawning.", safety_system.current_frame)).start()
                ...
                if not math_problem_state['active'] and not move_aside_state['given'] and is_phone_detected:
                    ...
                    if current_time - last_alert_time['phone_call_voice'] > CONFIG['VOICE_ALERT_COOLDOWN_SECONDS']:
                        ...
                        # CORRECTED CALL
                        threading.Thread(target=send_email, args=("Phone Call Alert!", "Driver has been detected using a phone.", safety_system.current_frame)).start()
                ...
                if not math_problem_state['active'] and not move_aside_state['given'] and is_face_missing:
                    ...
                    if current_time - last_alert_time['camera_blocked_voice'] > CONFIG['VOICE_ALERT_COOLDOWN_SECONDS']:
                        ...
                        # CORRECTED CALL
                        threading.Thread(target=send_email, args=("Camera Alert!", "Driver face is not visible or camera is blocked.", safety_system.current_frame)).start()
                ...
                if not math_problem_state['active'] and not move_aside_state['given']:
                    if (drowsiness_count >= CONFIG['STOP_VEHICLE_COUNT_THRESHOLD'] or
                        ...
                        if current_time - last_alert_time['stop_vehicle_alert'] > CONFIG['STOP_VEHICLE_ALERT_COOLDOWN']:
                            ...
                            # CORRECTED CALL
                            threading.Thread(target=send_email, args=("CRITICAL SAFETY ALERT!", "Driver has repeatedly shown signs of distraction/drowsiness. Advise to stop vehicle.", safety_system.current_frame)).start()
                ...
                # Update system stats and current frame
                with safety_system.lock:
                    safety_system.current_frame = frame.copy()
                    safety_system.system_stats.update({
                        'drowsiness_count': drowsiness_count,
                        'yawn_count': yawn_count,
                        'phone_use_count': phone_use_count,
                        'status': status_text
                    })
...
def generate_frames():
    ...
    while safety_system.is_running:
        with safety_system.lock:
            if safety_system.current_frame is not None:
                ret, buffer = cv2.imencode('.jpg', safety_system.current_frame)
                if ret:
...
def start_system():
    if not safety_system.is_running:
        safety_system.cap = cv2.VideoCapture(0)
        if not safety_system.cap.isOpened():
            return jsonify({'success': False, 'message': 'Cannot open camera'})
        
        safety_system.is_running = True
        detection_thread = threading.Thread(target=detection_loop)
        detection_thread.daemon = True
        detection_thread.start()
        
        return jsonify({'success': True, 'message': 'Safety system started'})
    else:
        return jsonify({'success': False, 'message': 'System is already running'})

@app.route('/stop_system', methods=['POST'])
def stop_system():
    if safety_system.is_running:
        safety_system.is_running = False
        stop_buzzer()
        if safety_system.cap:
            safety_system.cap.release()
        safety_system.system_stats = {
            'drowsiness_count': 0,
            'yawn_count': 0,
            'phone_use_count': 0,
            'status': 'IDLE',
        ...
        return jsonify({'success': True, 'message': 'Safety system stopped'})
    else:
...
def system_stats():
    with safety_system.lock:
        stats = safety_system.system_stats.copy()
    stats['is_running'] = safety_system.is_running
